...
The original prompts are in `backend/prompts.json` (one entry per call: `rules`, the five section keys, `overview`, `tasks`). Edit them from the app under **Settings → Email Digest → Prompts**. Edits are saved to `users/<uid>/digestPrompts` and read at the start of every run, so the next run (scheduled or Run now) picks them up with no commit. A prompt with no saved edit uses the one in `prompts.json`, and **Reset to original** removes the edit. Changing `prompts.json` itself changes the originals for both the script and the editor.
